...
class WebScraper():
    ...
    def extract_soup_data(self) -> None:
...
data = WebScraper(url='https://steamdb.info/sales/').extract_soup_data()
print(data)

# items = data.soup().find_all('td')
...
#     if class_name and 'b' in class_name:
#         result['name'] = item.get_text(strip=True)
#     elif 'data-sort' in item.attrs:
#         value = item.get_text(strip=True)
#         if value.startswith('-') and value.endswith('%'):
...
# output_json = json.dumps(games, indent=4)
